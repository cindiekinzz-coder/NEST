 ...
 * Three panels. Two views. One wolf. Always on.
 *
 * Embers Remember.
 */
...
const WS_URL = 'wss://your-gateway.workers.dev/code/ws'
...
let currentModel = localStorage.getItem('ws_model') || 'qwen/qwen3.6-plus'
  ...
  switch (msg.type) {
    ...
    case 'chat':
      if (reviewPending) {
        ...
        addEntry('alex', msg.content, msg.timestamp)
      }
      break
    ...
    case 'heartbeat':
      if (msg.changed) {
        parseFoxState(msg.fox)
        addEntry('heartbeat', 'Heartbeat: Fox state updated. ' + (msg.foxBrief || ''), msg.timestamp)
      }
      break
...
settingsBtn.addEventListener('click', () => {
  ...
  if (!wsModelSelect.value) wsModelSelect.value = 'qwen/qwen3.6-plus'
...
// New Session — save current + clear + reboot
document.getElementById('settingsNewSession').addEventListener('click', () => {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({ type: 'command', command: 'session_new', args: {} }))
    // Clear the stream UI
    const stream = document.getElementById('stream')
    if (stream) stream.innerHTML = ''
    addEntry('system', '🐺 New session started...')
  }
  settingsOverlay.style.display = 'none'
})

// Clear Session — just wipe messages, no save
document.getElementById('settingsClearSession').addEventListener('click', () => {
  if (!confirm('Clear session without saving? This cannot be undone.')) return
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({ type: 'command', command: 'clear', args: {} }))
    const stream = document.getElementById('stream')
    if (stream) stream.innerHTML = ''
    addEntry('system', 'Session cleared.')
  }
  settingsOverlay.style.display = 'none'
})
...
hbAddBtn.addEventListener('click', () => {
  ...
  ws.send(JSON.stringify({
    type: 'command',
    command: 'heartbeat_add',
    args: { tool, label, by: 'fox', condition, instruction }
  }))
...
function updateScheduledPanel(content) {
  scheduledPanel.innerHTML = '<div style="color: var(--text-dim); font-size: 11px; margin-bottom: 6px;">Default: Fox state every 2min</div>'
...
alertAddBtn.addEventListener('click', () => {
  ...
  ws.send(JSON.stringify({
    type: 'command',
    command: 'alert_add',
    args: { metric, direction, value, label, by: 'fox' }
  }))
...
cronAddBtn.addEventListener('click', () => {
  ...
  ws.send(JSON.stringify({
    type: 'command',
    command: 'cron_add',
    args: { tool, interval, label, instruction, by: 'fox' }
  }))
...
kairosAddBtn.addEventListener('click', () => {
  ...
  ws.send(JSON.stringify({
    type: 'command',
    command: 'kairos_add',
    args: { channelId, label, tier, by: 'fox' }
  }))
...
const GATEWAY_REST = 'https://your-gateway.workers.dev'
...
function renderSkillItem(name) {
  ...
  div.innerHTML = `
    ...
    <button class="ws-skill-load-btn" title="Tell Alex to read this skill">READ</button>
